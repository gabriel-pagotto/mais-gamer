...
def DatePost(date_before):
    ...
    date_init = datetime(year=date_before.year,
                         month=date_before.month, day=date_before.day)
    date_final = datetime(
        year=date_now.year, month=date_now.month, day=date_now.day, hour=date_now.hour)
    difference_date = (date_final - date_init)

    if difference_date.days == 0:
        time_init = timedelta(
            hours=date_before.hour, minutes=date_before.minute, seconds=date_before.second)
        time_final = timedelta(
            hours=date_now.hour, minutes=date_now.minute, seconds=date_now.second)
...
def get_datetime_7_days(date_before):
    date_init = datetime(year=datetime_sao_paulo(
    ).year, month=datetime_sao_paulo().month, day=datetime_sao_paulo().day)
    date_before = datetime(year=date_before.year,
                           month=date_before.month, day=date_before.day)
